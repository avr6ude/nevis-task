...
`/api`. `npm test` runs 50 tests: chart mapping, expand and collapse, the API
boundary, and axe against the rendered table and chart.
...
and every node's `values` has to line up with `months`. Three query params
forward from the page URL to check the states by hand: `/?delay=6000` for
loading, `/?fail=1` for the error, `/?bad=1` for a malformed payload.

## Accessibility

The table is a full treegrid, more than the brief asks for. Rows carry
`aria-level` and `aria-expanded`, and every month is its own grid cell, so Left
and Right walk across the year and a keyboard user reaches December without a
mouse. React Aria's `Tree` gives the hierarchy but puts a whole row in one cell,
which is why the keyboard handling is written by hand.

The chart's SVG is hidden from assistive tech and the same figures sit beside it
in a visually hidden table with row and column headers. The numbers are the
reason the chart exists, so they are not left mouse-only in a tooltip.
...
Company May then reads 278. Leaf numbers are taken as given, so if those are
wrong the whole thing is. A parent should carry a total that agrees with its
...
- Let a row be scoped directly, so opening two branches does not fall back to
  the company.
- Reconcile the totals with whoever owns the data.
- Keep the expanded rows in the URL so a drilled-in view can be shared.
- Give the tree a uniform `children[] + kind` shape. `getChildren` reads
  `branches ?? employees ?? channels`, which leaks the data model into the
  domain layer.
- Virtualise the table and load subtrees on demand if the data grows.
